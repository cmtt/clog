...
/**
 * @function
 * Formats a text with the provided attributes with ANSI control characters.
 *
...
var version = '0.1.6-proposal'
  ...
  , styles = {
    'log'   : {
      color : 'off',
      text : null /** When text === null, nothing will be shown at this line */
    },
    'error' : {
...
Clog.prototype.log = function (type, msg) {
  ...
  if (type === 'error') {
    if (args[msgIdx] === null) {
      (type = 'success') && args.splice(msgIdx,1);
    } else if (args[msgIdx] instanceof Error) {
      (args[msgIdx] = format(args[msgIdx].message,'red'));
    }
  }
  ...
  if (typeof style === TYPE_OBJECT) {
    if (typeof style.text === TYPE_STRING || style.text === null) {
      label = style.text;
      if (typeof style.color === TYPE_STRING) {
        label = format(label, style.color);
      }
      (style.text !== null) && pre.push(label);
    } else {
      pre.push(type);
    }
  } else {
    pre.push ( format(label,defaultStyle) );
  }
  ...
  if (showModule === true) {
    args.shift();
    pre.push(format(args[0],moduleStyle)); 
  }
